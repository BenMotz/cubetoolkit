...
def add_member(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            results = Member.objects.filter(email=instance.email).order_by(
                "name"
            )
            if results:
                for result in results:
                    logger.info(
                        "Member with id %s %s <%s> already in database"
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                "Added member: {0}".format(instance.number),
            )
    ...
    if request.user.has_perm("toolkit.write"):
        ...
        if vol and vol.active:
            messages.add_message(
                request,
                messages.ERROR,
                "Can't delete active volunteer {0} ({1}). Retire them first.".format(
                    member.name, member.number
                ),
            )
            logger.info(
                "Attempt to delete active volunteer {0} {1} <{1}>".format(
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                "Deleted member: {0} ({1})".format(member.number, member.name),
            )
            logger.info(
                "Member {0} {1} <{2}> deleted by {3}".format(
                    member.number,
                    member.name,
                    member.email,
        ...
        if vol and vol.active:
            ...
            logger.info(
                "Futile attempt by active volunteer {0} <{1}> to delete themselves".format(
                    vol.member.name, vol.member.email
                )
            )
        ...
        if confirmed == "yes":
            ...
            logger.info(
                "Member {0} {1} <{2}> self-deleted".format(
                    member.number, member.name, member.email
                )
            )
            member.delete()
...
def edit_member(request, member_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if hasattr(member, "volunteer") and hasattr(
                member.volunteer, "user"
            ):
                # For volunteers, keep user name in sync with member name
                logger.info(
                    "Updating user name '%s' to '%s'"
        ...
        if confirm == "yes":
            if action == "opt-in":
                ...
                messages.add_message(
                    request,
                    messages.SUCCESS,
                    "Thank you {0} for opting in to continue to receive our emails".format(
                        member.name
                    ),
                )
            else:  # opt-out
                ...
                messages.add_message(
                    ...
                    messages.SUCCESS,
                    (
                        "We are sorry to see you have opted out. "
                        "If you do not opt-in by 25 May 2018 "
                        "we will delete your membership from our records."
                    ),
                )
            member.save()
