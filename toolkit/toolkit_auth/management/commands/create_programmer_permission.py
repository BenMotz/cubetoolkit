...
def _create_programme_role(self):
    ...
    if role:
        self.stdout.write(
            self.style.WARNING("%s role already exists" % role_name)
        )
    else:
...
def _create_programme_group(self):
    ...
    if Group.objects.filter(name=group_name):
        self.stdout.write(
            self.style.WARNING("%s group exists already" % group_name)
        )
    else:
        Group.objects.create(name=group_name)
        self.stdout.write(self.style.SUCCESS("Created %s group" % group_name))
...
def _create_programme_permission():
    ...
    programmer_permissions = auth_models.Permission.objects.get_or_create(
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("Permissions for %s group set" % group_name)
        )
