...
def edit_diary_list(request, year=None, day=None, month=None):
    ...
    context["rooms"] = (
        list(Room.objects.all()) if settings.MULTIROOM_ENABLED else [None]
    )
...
def add_showing(request, event_id):
    ...
    if clone_showing_form.is_valid():
        ...
        logger.info(
            '{0} added showing on {1} for event "{2}"'.format(
...
def edit_showing(request, showing_id=None):
    ...
    if request.method == "POST":
        ...
        if (
            confirmed
            and terms_word_count < settings.PROGRAMME_EVENT_TERMS_MIN_WORDS
        ):
            messages.add_message(
                request,
                messages.ERROR,
    ...
    clone_showing_form = diary_forms.CloneShowingForm(
        initial={"clone_start": clone_start}
    )
...
class EditEventView(PermissionRequiredMixin, View):
    ...
    def post(self, request, event_id):
        ...
        if terms_ok and form.is_valid() and media_form.is_valid():
            ...
            logger.info(
                '{0} updated event "{1}" (id:{2})'.format(
                    request.user.last_name, event.name, event_id
                )
            )
...
def delete_showing(request, showing_id):
    ...
    if showing.in_past():
        ...
        logging.info(
            "{0} deleted showing id {1} (for event id {2})".format(
                request.user, showing_id, showing.event_id
            )
        )
        messages.add_message(
            request,
...
class EditRotaView(PermissionRequiredMixin, View):
    ...
    def post(self, request, year=None, day=None, month=None):
        ...
        if (
            settings.VENUE["show_user_management"]
        ...
        logger.info(
            "{0} updated role id {1} (#{2}) for showing "
            "'{3}' '{4}' -> '{5}' ({6})".format(
                request.user.last_name,
                rota_entry.role_id,
                rota_entry.rank,
...
def edit_showing_rota_notes(request, showing_id):
    ...
    logger.debug(
        '%s editing rota notes for showing "%s"' % (request.user, showing)
    )
    old_notes = showing.rota_notes
    if showing.in_past():
        logger.error(
            "%s attempted to edit rota notes for %s in the past"
        ...
        form.save()
        logger.debug(
            'rota_notes "%s" -> "%s"' % (old_notes, showing.rota_notes)
        )
    else:
        logger.error("form %s not valid" % form)
        return HttpResponse(
            "Rota notes edit form not valid!",
            status=500,
            content_type="text/plain",
        )
