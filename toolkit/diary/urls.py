...
from toolkit.diary.edit_views import (
    EditEventView, EditRotaView, cancel_edit, edit_diary_list,
    edit_diary_calendar, edit_diary_data, edit_showing,
    edit_showing_rota_notes, edit_ideas, add_showing, delete_showing,
    add_event, edit_event_templates, edit_event_tags, edit_roles,
    view_event_field, view_rota_vacancies, set_edit_preferences,
    get_messages, printed_programme_edit, view_force_error)
from toolkit.diary.public_views import (
    ArchiveIndex, ArchiveYear, ArchiveMonth, ArchiveSearch, view_diary,
...
programme_urls = [
    ...
    url(r'^view/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/?$',
        view_diary, name="day-view"),
    ...
    url(r'^showing/id/(?P<showing_id>\d+)/$',
        view_showing, name="single-showing-view"),
    # All showings for a given event
    url(r'^event/id/(?P<event_id>\d+)/$', view_event,
        name="single-event-view"),
    url(r'^event/(?P<event_slug>[\w\-_]*),(?P<event_id>\d+)/$',
        view_event, name="single-event-view-with-slug"),
    # As above, for legacy event ID:
    url(r'^event/oldid/(?P<legacy_id>\d+)/$',
        view_event, name="single-event-view-legacyid"),

    # Archive:
    url(r'^archive/$', ArchiveIndex.as_view(), name="archive-view-index"),
    url(r'^archive/(?P<year>\d{4})/$',
        ArchiveYear.as_view(), name="archive-view-year"),
    url(r'^archive/(?P<year>\d{4})/(?P<month>\d{1,2})/$',
        ArchiveMonth.as_view(), name="archive-view-month"),
    # Search
    url(r'^archive/search/$', ArchiveSearch.as_view(), name="archive-search"),

    # Get JSON describing events on a given (single) date:
    url(r'^view/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})/json$',
        view_diary_json, name="day-view-json"),

    # RSS feed
    url(r'^rss/$', toolkit.diary.feeds.BasicWhatsOnFeed(),
        name="view-diary-rss"),
]
...
diary_urls = [
    ...
    url(r'^edit/calendar/?$', edit_diary_calendar,
        name="diary-edit-calendar"),
    url(r'^edit/calendar/(?P<year>\d{4})/(?P<month>\d{1,2})/?$',
        edit_diary_calendar,),
    url(r'^edit/calendar/(?P<year>\d{4})/(?P<month>\d{1,2})/'
        r'(?P<day>\d{1,2})/?$',
        edit_diary_calendar),
    ...
    url(r'^edit/(?P<year>\d{4})/(?P<month>\d{1,2})/?$',
        edit_diary_list, name="month-edit", ),
    url(r'^edit/(?P<year>\d{4})/(?P<month>\d{1,2})/(?P<day>\d{1,2})',
        edit_diary_list, name="day-edit", ),
    ...
    # Edit an event
    url(r'^edit/event/id/(?P<event_id>\d+)/$',
        EditEventView.as_view(), name="edit-event-details"),
    # Edit a showing (includes delete / add a new showing)
    url(r'^edit/showing/id/(?P<showing_id>\d+)/$',
        edit_showing, name="edit-showing"),
    # Edit rota notes for a showing
    url(r'^edit/showing/id/(?P<showing_id>\d+)/rota_notes/$',
        edit_showing_rota_notes, name="edit-showing-rota-notes"),
    # Edit ideas
    url(r'^edit/ideas/(?P<year>\d{4})/(?P<month>\d{1,2})/$',
        edit_ideas, name="edit-ideas"),
    # Add a new showing (to an existing event) - submission URL for
    # edit-showing
    url(r'^edit/event/id/(?P<event_id>\d+)/addshowing$',
        add_showing, name="add-showing"),
    # Delete a showing
    url(r'^edit/showing/id/(?P<showing_id>\d+)/delete$',
        delete_showing, name="delete-showing"),
    # Add a new event + showing
    url(r'^edit/event/add$', add_event, name="add-event"),

    # Edit event types
    url(r'^edit/eventtemplates/', edit_event_templates,
    ...
    # View rota
    url(r'^(?P<field>rota|copy|terms|copy_summary)(/|/(?P<year>\d{4})/'
    ...
    url("^printedprogrammes$", printed_programme_edit,
        name="edit-printed-programmes", kwargs={'operation': 'edit'}),
    url("^printedprogrammes/add$", printed_programme_edit,
        name="add-printed-programme", kwargs={'operation': 'add'}),
